...
import com.dineshkb.gnu.controller.{ConfigurationItemController, TagController}
...
class TagResourceAssembler extends SimpleRepresentationModelAssembler[Tag] {
  ...
  override def addLinks(resource: EntityModel[Tag]): Unit = {
    ...
    resource.add(linkTo(methodOn(classOf[ConfigurationItemController]).getItemsWithTag(resource.getContent.no)).withRel("configurationitems"))
  }
